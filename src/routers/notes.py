...
async def update_note(note_title: str, data: str, current_user: dict = Depends(get_current_user)):
    ''' update note '''
...
@router.delete('/{title}')
async def delete_note(title: str, current_user: dict = Depends(get_current_user)):
    ''' delete note '''
